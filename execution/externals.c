...
/*   Updated: 2026/06/05 14:35:28 by olmatske         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	child_exec(t_cmd *cmd, char *path, char **arr)
{
	...
	{
		ft_puterr("Is a directory", path, NULL, 1);
	...
	saved_errno = errno;
	ft_puterr(path, strerror(saved_errno), NULL, 1);
	free_split(arr);
...
int	exec_external(t_shell *shell, t_cmd *cmd, t_env *env)
{
	...
	if (!arr)
		return (ft_puterr("malloc failure", NULL, NULL, 1), 1);
	path = cmd->args[0];
	if (!decide_path(cmd))
		path = resolve_path(shell, cmd, arr);
	if (!path)
		return (free_split(arr),
			ft_puterr(cmd->args[0], C, NULL, 0), 127);
	pid = fork();
	if (pid < 0)
		return (free_split(arr), ft_puterr(F, NULL, NULL, 1), 1);
	if (pid == 0)
