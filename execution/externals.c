...
/*   Updated: 2026/06/04 21:31:01 by olmatske         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_abortion(int status)
{
	...
	if (WIFSIGNALED(status))
	{
		if (WTERMSIG(status) == SIGQUIT)
			ft_putendl_fd(C_RED"Quit (core dumped)"C_RESET, STDERR_FILENO);
...
static void	child_exec(t_cmd *cmd, char *path, char **arr)
{
	...
	saved_errno = errno;
	fprintf(stderr, C_RED"%s %s: %s\n"C_RESET, M, path, strerror(saved_errno));
	free_split(arr);
...
int	exec_external(t_shell *shell, t_cmd *cmd, t_env *env)
{
	...
	if (!path)
		return (free_split(arr),
			fprintf(stderr, C_RED"%s: %s"C_RESET, cmd->args[0], C), 127);
	pid = fork();
	if (pid < 0)
